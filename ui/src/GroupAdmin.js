...
function GroupAdmin(props) {
  ...
  function openRemoveUserPage(groupUser) {
    history.push(`deleteuserfromgroupconfirmation?groupUserId=${groupUser.id}&groupName=${props.groupName}&groupId=${props.groupId}&userEmail=${groupUser.userEmail}`);
  }

  useEffect(() => {
    async function fetchAllUsersInGroup() {

      const allGroupMembersResponse = await fetch(`/api/userGroupMembers/findByGroup/${props.groupId}`)
      ...
      const usersInGroupRows = await allUsersInGroup.map((groupUser, index) => (
        <tr className="table__row" key={index}>
          ...
          <td className="table__cell">
            <button onClick={() => openRemoveUserPage(groupUser)} type="button">Remove</button>
          </td>
        </tr>
      ));
  ...
  return (
    <>
      ...
      {!props.flashMessageUntil && <Announcer text={`Users in group: ${props.groupName}`} />}
